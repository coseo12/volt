...
이미 정제된 노트에 추가 관찰이 생기면:

- **기본 경로**: close 된 원본 이슈에 코멘트로 추가 관찰을 남김. 관리자가 노트 말미에 `## 보강 (YYYY-MM-DD 후속)` 섹션을 추가하고 이슈에 커밋 해시를 회신.
- **작정한 편집**: PR 로 노트 직접 수정 제안.
- 이슈는 재오픈하지 않음. 이력은 노트의 날짜별 보강 섹션과 git log 로 보존.
- 상세 절차는 [AGENTS.md](./AGENTS.md#사후-보강-정제-완료-노트의-갱신) 참고.
